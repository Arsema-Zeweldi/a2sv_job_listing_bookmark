...
const JobCard = (job: JobListingInterface) => {
  ...
  const handleCreateBookmark = (eventId: string) => {
    createBookmarkTrigger(eventId)
      .unwrap()
      .then(() => {
        setBookmarks((currentBookmarks) => [...currentBookmarks, eventId]);
      });
  };
  useEffect(() => {
    setIsClient(true);
    ...
    <div data-testid={`job-card-${job.id}`}>
      <div className="border-gray-300 border rounded-4xl mr-80 ml-30 mt-5 mb-5 p-5 hover:bg-gray-300">
        <div className="p-2 flex gap-6">
          ...
          >
            {bookmarks.includes(job.id) ? (
              <RiBookmarkFill className="text-2xl cursor-pointer" />
            ) : (
              <RiBookmarkLine className="text-2xl cursor-pointer" />
            )}
          </button> */}
        </div>
      </div>
